...
if ( !class_exists( 'tm_mega_menu_widget_manager' ) ) :
 ...
 */
class tm_mega_menu_widget_manager {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_tm_mega_menu_save_widget',    array( $this, 'save_widget' ) );
		add_action( 'wp_ajax_tm_mega_menu_update_columns', array( $this, 'update_columns' ) );
		add_action( 'wp_ajax_tm_mega_menu_delete_widget',  array( $this, 'delete_widget' ) );
		add_action( 'wp_ajax_tm_mega_menu_add_widget',     array( $this, 'add_widget' ) );
	 ...
	 * @since 1.0.0
	 */
	public function persist_tm_mega_menu_widget_settings( $instance, $new_instance, $old_instance, $that ) {
	...
	public function register_sidebar() {
		...
		register_sidebar(
			array(
				'id'          => 'tm-mega-menu',
				'name'        => __( 'tm mega menu Widgets', 'tm-mega-menu' ),
				'description' => __( 'Do not manually edit this area.', 'tm-mega-menu' )
			)
		);
	}
	...
	public function show_widget_form() {
		...
		check_ajax_referer( 'tm_mega_menu', '_wpnonce' );
	...
	public function save_widget() {
		...
		$message = $saved ? sprintf( __( 'Saved %s', 'tm-mega-menu' ), $id_base ) : sprintf( __( 'Failed to save %s', 'tm-mega-menu' ), $id_base );

		do_action( 'tm_mega_menu_after_widget_save' );
	...
	public function update_columns() {
		...
		$message   = $updated ? sprintf( __( 'Updated %s (new columns: %d)', 'tm-mega-menu' ), $widget_id, $columns ) : sprintf( __( 'Failed to update %s', 'tm-mega-menu' ), $widget_id );
	...
	public function add_widget() {
		...
		if ( $added && is_array( $added ) ) {
			...
			$result = array(
				'type'    => 'error',
				'content' => sprintf( __( 'Failed to add %s to %d', 'tm-mega-menu' ), $id_base, $menu_item_id )
			);
		}

		do_action( 'tm_mega_menu_after_widget_add' );
		do_action( 'tm_mega_menu_save' );
	...
	public function delete_widget() {
		...
		$message   = $deleted ? sprintf( __( 'Deleted %s', 'tm-mega-menu' ), $widget_id ) : sprintf( __( 'Failed to delete %s', 'tm-mega-menu' ), $widget_id );
	...
	public function move_widget() {
		...
		$message        = $moved ? sprintf( __( 'Moved %s to %d (%s)', 'tm-mega-menu' ), $widget_to_move, $position, json_encode( $moved ) ) : sprintf( __( 'Failed to move %s to %d', 'tm-mega-menu' ), $widget_to_move, $position );
	...
	public function show_widget( $id ) {
		...
		$params[0][ 'before_title' ]  = apply_filters( 'megamenu_widgets_before_title', '<h4 class="mega-block-title">' );
		$params[0][ 'after_title' ]   = apply_filters( 'megamenu_widgets_after_title', '</h4>' );
		$params[0][ 'before_widget' ] = apply_filters( 'megamenu_widgets_before_widget', '' );
		$params[0][ 'after_widget' ]  = apply_filters( 'megamenu_widgets_after_widget', '' );
	...
	public function get_widget_html( $title, $widget_id, $cols, $total_cols ) {
		...
		$cols       = esc_attr( $cols );
		$total_cols = esc_attr( $total_cols );
		$widget_id  = esc_attr( $widget_id );

		return '<div class="widget" id="' . $widget_id . '" data-columns="' . $cols . '" data-total-columns="' . $total_cols . '" data-widget-id="' . $widget_id . '"><div class="widget-top"><div class="widget-title"><h4>' . $title . '</h4><span class="spinner" style="display: none;"></span></div>
		<div class="widget-title-action">
			<div class="widget-columns-resize">
			<a class="widget-option widget-contract widget-resize" data-action="contract">-</a>
			<a class="widget-option widget-expand widget-resize" data-action="expand">+</a>
	...
	private function get_sidebar_widgets() {
		...
		return isset( $sidebar_widgets[ 'tm-mega-menu' ] ) ? $sidebar_widgets[ 'tm-mega-menu' ] : false;
	}
	...
	private function set_sidebar_widgets( $widgets ) {
		...
		$sidebar_widgets                       = wp_get_sidebars_widgets();
		$sidebar_widgets[ 'tm-mega-menu' ] = $widgets;
