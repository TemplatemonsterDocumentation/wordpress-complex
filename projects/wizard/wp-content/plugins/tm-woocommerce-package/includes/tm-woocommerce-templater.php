 ...
 * @since 1.0.0
 */
class TM_WooCommerce_Templater {
	...
	public function __construct() {
		...
		require_once tm_wc()->plugin_dir( 'includes/tm-woocommerce-template-callbacks.php' );

		$callbacks = new TM_WooCommerce_Template_Callbacks();
		 ...
		 */
		$this->replace_data = apply_filters( 'tm_woocommerce_templater_callbacks', $data );
	...
	public function get_template_by_name( $template, $shortcode ) {
		...
		$file       = '';
		$default    = tm_wc()->plugin_dir( 'templates/shortcodes/' . $shortcode . '/default.tmpl' );
		 ...
		 */
		$content = apply_filters( 'tm_woocommerce_package_fallback_template', '' );
		...
		if ( file_exists( $upload_dir . $subdir ) ) {
			...
			$file = $theme_template;
		} elseif ( file_exists( tm_wc()->plugin_dir( $subdir ) ) ) {
			$file = tm_wc()->plugin_dir( $subdir );
		} else {
			$file = $default;
		}
 ...
 */
function tm_woocommerce_templater() {
	return TM_WooCommerce_Templater::get_instance();
}

tm_woocommerce_templater();
