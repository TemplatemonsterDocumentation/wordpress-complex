	<h3>Easy Twitter Feed</h3>

	<p>This widget is used to setup and display twitter feed box on the website.</p>

    <figure class="img-polaroid">
        <img src="projects/<?php echo $project;?>/img/widgets/twitter-widget.png">
    </figure>

	<ul class="marked-list">
    <li>
        <dl class="inline-term">
            <dt>Title</dt>
            <dd>
				This property specifies the widget title
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Widget ID</dt>
            <dd>
                Here you can define the Widget ID to show the account images on your website
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Screen name</dt>
            <dd>
                Here you can set a twitter account name
            </dd>
        </dl>
    </li>

    <li>
        <dl class="inline-term">
            <dt>Tweet Limit</dt>
            <dd>
                Here you can define a number of twitts to display (up to 20)
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Show Replies</dt>
            <dd>
                Here you can define whether to show or hide the replies
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Widget Width</dt>
            <dd>
                This property defines the width of twitter widget block
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Widget Height</dt>
            <dd>
                This property defines the height of the twitter widget block
            </dd>
        </dl>
    </li>

    <li>
        <dl class="inline-term">
            <dt>Twitter Widget Theme</dt>
            <dd>
                Here you can select a theme for the widget
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Widget Link Color</dt>
            <dd>
                This property defines the link color of the twitter widget block
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Show Twitter Widget Header</dt>
            <dd>
                Here you can define whether to show or hide the header in the twitter widget
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Show Twitter Widget Footer</dt>
            <dd>
                Here you can define whether to show or hide footer inside the twitter widget
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Show Twitter Widget Border</dt>
            <dd>
                Here you can define whether to show or hide border in the twitter widget
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Twitter Widget Border Color</dt>
            <dd>
                This property defines the border color of the twitter widget block
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Show Twitter Widget Scrollbar</dt>
            <dd>
                Here you can define whether to display or remove the scrollbar in the twitter widget. Scrollbar setting will work only @ Tweet Limit "default".
            </dd>
        </dl>
    </li>
    <li>
        <dl class="inline-term">
            <dt>Use Twitter Widget Background Color</dt>
            <dd>
                This property defines whether to show or hide the Background Color for the twitter widget
            </dd>
        </dl>
    </li>
</ul>